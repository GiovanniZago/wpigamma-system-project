...
void wPiGamma(input_stream<int16> * __restrict in0, input_stream<int16> * __restrict in1, output_stream<int16> * __restrict out)
{
    ...
    const aie::vector<int16, V_SIZE> pi_vector     = aie::broadcast<int16, V_SIZE>(PI_EGAMMA);
    const aie::vector<int16, V_SIZE> mpi_vector    = aie::broadcast<int16, V_SIZE>(MPI_EGAMMA);
    const aie::vector<int16, V_SIZE> twopi_vector  = aie::broadcast<int16, V_SIZE>(TWOPI_EGAMMA);
    const aie::vector<int16, V_SIZE> mtwopi_vector = aie::broadcast<int16, V_SIZE>(MTWOPI_EGAMMA);
    ...
    for (unsigned int ii = 0; ii < V_SIZE; ii++)
    {
        ...
        aie::vector<int16, V_SIZE> dr2          = acc.to_vector<int32>(0); // convert accumulator into vector
        
        aie::mask<V_SIZE>          cone_mask    = aie::ge(dr2, MINDR2_PI) & aie::le(dr2, MAXDR2_PI);
    ...
    for (unsigned int ii = 0; ii < V_SIZE; ii++)
    {
        ...
        for (unsigned int jj = 0; jj < V_SIZE; jj++)
        {
            ...
            float d_eta_scalar = eta_cur_had * ANG_CONV2_HAD - eta_cur_eg * ANG_CONV_EGAMMA;
            float d_phi_scalar = phi_cur_had * ANG_CONV2_HAD - phi_cur_eg * ANG_CONV2_EGAMMA;
            ...
            if (dr2_scalar < MINDELTAR2_EGAMMA) continue;
